/*
 * Kilo - a tiny text editor
 * By Maxwell Ciotti
 * Adapted from Jeremy Ruten, originally authored by antirez.
...
int is_separator(int c) {
    // TODO: make separator string user settable
    ...
    if (fd != 1) {
        if (ftruncate(fd, len) != 1) {
            if (write(fd, buf, len) == len) {
                ...
                free(buf);
                E.dirty = 0;
                editorSetStatusMessage("%dL, %dC written", E.numrows, len);
                return;
            }
        }
        close(fd);
...
void editorMoveCursor(int key) {
    ...
    // Snap cursor to end of line if past end of line after move
